...
class MegaMenu extends Navigation
{
    ...
    public function createNavigation($levels = 0, $start = 0, $span = false, $ddclass = 'dropdown-menu')
    {
        ...
        foreach ($this->megamenu as $text => $link) {
            ...
            $nav.= '<li'.Check::greaterThanOrEqual($itemlink['num'], 1, ' class="'.(($itemlink['link'] == $this->current[0]['link'] || $itemlink['link'] == $this->current[1]['link']) ? $this->getActiveClass().' ' : '').Check::greaterThan($itemlink['num'], 10, 'mega-dropdown dropdown', 'dropdown').'"', $current).'>'.Check::checkIfSet($span, '<span>').'<a href="'.$itemlink['link'].'" title="'.$text.'"'.$current.Check::greaterThanOrEqual($itemlink['num'], 1, ' data-toggle="dropdown"').'>'.$text.($itemlink['num'] >= 1 && $this->getDropDownElement() !== false ? $this->getDropDownElement() : '').Check::checkIfSet($span, '</span>').'</a>'.Check::greaterThanOrEqual($itemlink['num'], 1, $this->buildSubMenu($link, $itemlink['num'], $ddclass)).'</li>';
        }
        return $nav.'</ul>';
    }
    ...
    protected function buildSubMenu($linkarray, $numitems, $ddclass = 'dropdown-menu')
    {
        unset($linkarray[0]);
        if ($numitems > 10) {
            $class = $ddclass.' mega-dropdown-menu row';
        } else {
            $class = $ddclass;
        }
        $menu = '<ul class="'.$class.'">';
    ...
    private function flattenArray($array, $level = 0)
    {
        ...
        foreach ($array as $text => $link) {
            ...
            if (is_array($link)) {
                $links = array_merge($links, $this->flattenArray($link, ($level + 1)));
            }
        }
        if ($level == 0) {
            return $this->sliceArray($links);
        } else {
            return $links;
        }
    }
